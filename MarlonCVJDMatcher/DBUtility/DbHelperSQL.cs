...
namespace Tclywork.DBUtility
{
    public abstract class DbHelperSQL
    {
        //数据库连接字符串(web.config来配置)，可以动态更改connectionString支持多数据库.
        public static string connectionString = ConfigurationManager.ConnectionStrings["DBString"] == null ? ConfigurationManager.AppSettings["DBString"] : ConfigurationManager.ConnectionStrings["DBString"].ConnectionString;
        public DbHelperSQL()
        {
            try
            {
            ...
            {
                connectionString = ConfigurationManager.AppSettings["DBString"];
            }
        }
        ...
        #endregion 判断是否存在某表的某个字段

        #region 获取某字段的最小值+1

        public static int GetMinID(string FieldName, string TableName, string Where = "")
        {
            string strsql = string.Empty;
            if (string.IsNullOrEmpty(Where))
                strsql = "select min(" + FieldName + ")+1 from " + TableName;
            else
                strsql = "select min(" + FieldName + ")+1 from " + TableName + " where " + Where;
        ...
        public static int GetMaxID(string FieldName, string TableName, string Where = "")
        {
            ...
            if (string.IsNullOrEmpty(Where))
                ...
                strsql = "select max(" + FieldName + ")+1 from " + TableName + " where " + Where;
            object obj = DbHelperSQL.GetSingle(strsql);
            if (obj == null)
            {
                return 1;
        ...
        #region 执行SQL语句，返回影响的记录数 设定超时时间
        ...
        #endregion 执行SQL语句，返回影响的记录数 设定超时时间
        ...
        #endregion 执行查询语句，返回SqlDataReader ( 注意：调用该方法后，一定要对SqlDataReader进行Close )
